                                            ...
                                            <div class="modal-dialog modal-dialog-scrollable" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLabel">Sewa Armada</h5>
                                                        ...
                                                        <thead class="thead-warning">
                                                            ...
                                                            <tbody>
                                                                @foreach($pricelist_sewa_armada as $pr)
                                                                    <tr id="row{{$pr->ID_PRICELIST }}">
                                                                        <th scope="row"><input type="checkbox" id="as{{$pr->ID_PRICELIST }}" ></th>
                                                                        <td>{{ $pr->NAMA_CATEGORY}}</td>
                                                                        <td>{{ $pr->TUJUAN_SEWA}}</td>
                                                                        <td>{{ $pr->PRICELIST_SEWA}}</td>
                                                                    </tr>
                                                                @endforeach
                                                            </tbody>
                                                    </table>
						                            ...
						                            <h4>Carts Total</h4>
						                                <table class="table table-bordered"> 
                                                            <tbody>
                                                                ...
                                                                <tr>
                                                                    <input type="hidden" name="total_payment" id="sisa_bayar">
                                                                        <th>Sisa Bayar
                                                                            <td id="sisa_bayar-val"></td>
                                                                </tr>
...
function getModal(event){
        if(event.keyCode==13){
            ...
            success : function(results){
                ...
                for(let i=0; i<results.pricelist_sewa_armada.length; i++){
                    $('#tabelproduk').DataTable().row.add([
                        '<input type="checkbox" id="as'+results.pricelist_sewa_armada[i]['ID_PRICELIST']+'">',
                        results.pricelist_sewa_armada[i]['NAMA_CATEGORY'],
                        results.pricelist_sewa_armada[i]['TUJUAN_SEWA'],
                        results.pricelist_sewa_armada[i]['PRICELIST_SEWA'],
                    ]).node().id = 'row'+results.pricelist_sewa_armada[i]['ID_PRICELIST'];
                    $('#tabelproduk').DataTable().draw();
                }
                $("#tambahModal").modal();
            },
    ...
    var products = <?php echo json_encode($pricelist_sewa_armada); ?>;

  function addRow(id){
      var index = getIndex(id);
      var id = products[index]["ID_PRICELIST"];
      var name = products[index]["NAMA_CATEGORY"];
      var price = products[index]["TUJUAN_SEWA"];
      var stock = products[index]["PRICELIST_SEWA"];
...
function money(text){
  ...
  // console.log(text);
    ...
    //   }
      ...
      var dp = (Number(percent)25/100) * total;
      document.getElementById("discount"+id).value = dp;
      document.getElementById("total"+id).value = total;
      document.getElementById("total-val"+id).innerHTML = money(total-dp);
      getTotal();
    };
    function getTotal(){
    ...
    //   // console.log(money(l));
      document.getElementById('dp').value = total_disc;
      document.getElementById('dp-val').innerHTML = money(total_disc);
      let k = (Number(total_p))-total_disc;
      if(k<=0){
        console.log('masuk if k<=0');
        document.getElementById('sisa_bayar').value = 0;
        document.getElementById('sisa_bayar-val').innerHTML = '0';
      }
      else{
        console.log('masuk if k!=0');
        document.getElementById('sisa_bayar').value = (total_p+(Number(total_p*0.1)))-total_disc;
        document.getElementById('sisa_bayar-val').innerHTML = money((total_p+(Number(total_p*0.1)))-total_disc);
      }
